...
const Entrance = () => {
  ...
  const [isLogin, setIsLogin] = useState({
    ...
    compon: <Lottie options={defaultOptionsSignup} height={500} width={500} />,
  });
  ...
  const handleToggle = () => {
    if (isLogin.login) {
      setIsLogin({
        login: false,
        compon: (
          <Lottie options={defaultOptionsSignup} height={500} width={500} />
        ),
      });
    } else {
      setIsLogin({
        login: true,
        compon: (
          <Lottie options={defaultOptionsLogin} height={500} width={500} />
        ),
      });
    }
  };
    ...
    <div className="w-full px-12 py-16">
      <div className="bg-white rounded flex w-full h-full">
        ...
        <div className="flex flex-col justify-center relative items-center space-y-3 w-1/2">
          <div
            className={`bg-pink-500  absolute top-0 w-full h-full z-10 ${
              !isLogin.login ? style["login-slide"] : style["signup-slide"]
            }`}
          >
            {/* Slider Window */}
            <div className={style["slider-window"]}>{isLogin.compon}</div>
          </div>
          <form className={style.form}>
            ...
            <p className={style.signin}>
              ...
              <Link onClick={handleToggle}>Signup here.</Link>{" "}
            </p>
          </form>
        </div>
        ...
        <div className=" flex flex-col justify-center items-center space-y-3 w-1/2 rounded-3xl">
          <form className={style.form}>
            ...
            <p className={style.signin}>
              ...
              <Link onClick={handleToggle}>Signin here.</Link>{" "}
            </p>
          </form>
        </div>
      </div>
