  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const { data } = await axios.post(
        "https://knit-kart.onrender.com/user/update-user",
        formData,
        {
          headers: {
  ...
  const deleteAd = async (id) => {
    try {
      const res = await axios.delete(
        `https://knit-kart.onrender.com/ad/deleteAd/${id}`
      );
  ...
  const getUserAds = async () => {
    ...
    try {
      const res = await axios.post(
        "https://knit-kart.onrender.com/ad/getUserAds",
        payload
      );
      console.log(res);
      setUserAds(res.data);
      ...
      <div className="w-screen h-fit flex bg-background text-text">
        ...
        <div className="flex flex-col md:w-1/5 w-2/12 py-7 pl-5 border-r">
          <div className="flex items-center space-x-4">
            <FaGear />
            <button className="font-bold text-2xl text-start md:block hidden">
              Settings
            </button>
          </div>
          <div className="flex flex-col ml-2 space-y-3 mt-11">
            ...
            >
              ...
              <button className="text-lg transition-colors duration-300 md:block hidden">
                Edit Profile
              </button>
            </div>
            <div
        ...
        <div className="w-full py-10 space-y-7">
          ...
          <div className="flex items-center justify-between md:w-9/12 w-10/12 mx-auto">
              ...
              {activeSection === "profile" ? (
                <div className="flex items-start flex-col">
                  ...
                  <div className="flex md:space-x-12 md:flex-row flex-col space-y-2">
                    <span
                      className="md:text-xl text-base flex items-center font-light cursor-pointer"
            ...
            <img
              ...
              alt="img"
              className="md:h-32 h-16 md:w-32 w-16 rounded-full"
            />
          </div>
